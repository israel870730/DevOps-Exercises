...
output "dns_privado_servidor" {
  description = "DNS privado de los servidores"
  value       = [ for instance in aws_instance.demo :
  "http://${instance.private_dns}:${var.puerto_servidor}"
  ]
}
...
output "ip_privada_servidor" {
  description = "IP privada IPV4 de los servidores"
