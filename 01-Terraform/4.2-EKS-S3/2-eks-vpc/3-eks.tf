...
resource "aws_security_group_rule" "eks_sg_https" {
  depends_on        = [module.eks]
  description       = "Allow https traffic to manage the cluster from the Jumbox"
  type              = "ingress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
...
#   policy_arn = "arn:aws:iam::aws:policy/AmazonSSMFullAccess" # AmazonSSMManagedInstanceCore
...
module "eks_kubernetes_addons" {
  ...
  # #external_dns_route53_zone_arns = ["arn:aws:route53:::hostedzone/Z02017111DVJY6OTSQZA4"]
  ...
  #   #values      = [file("${path.module}/values/lbcontroller_values.yaml")]
  # }
